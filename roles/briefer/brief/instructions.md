...
- The three **project context** files: `deliberate/context/product.md` for the product and Market (category, standards/protocols, technologies/trends); **`deliberate/context/competitors.md`** as the single source of truth for the named competitor roster, details, and official monitoring sources; and **`deliberate/context/ecosystem.md`** as the single source of truth for named dependencies, complements, channels, and movers (each `current`/`potential`), their details, and official monitoring sources. `competitors.md` grounds the **Competition** lens; `ecosystem.md` + the Market section in `product.md` ground the **Market** lens.
...
### Competition
For **each named competitor** in `competitors.md`, scan their **first-party** signals
...
### Market
Scan the wider **space and players around the project** (grounded in the roster, details, and sources in `ecosystem.md` — dependencies, complements, channels, movers — and the **Market** section in `product.md`:
...
- **Action items:** reason how those findings are actionable **for this project** — what decision or investigation is warranted, which finding motivates it, why it is valuable now, and what decision it would unlock. Include only genuinely warranted actions; an empty list is fine when the window was quiet. Classify each item by follow-through: an unresolved capability or experience decision is a **product & experience case**; an audience, positioning, pricing, packaging, channel, launch, or sales decision is a **market & commercial case**; a where-to-play, how-to-win, portfolio, priority, or resource commitment is a **strategy & portfolio case**; a build/buy/partner/adopt/integrate/migrate/standard/dependency choice is a **platform & ecosystem case**. Routine execution, instrumentation fixes, already-decided work, and bounded research are direct actions, not cases. Frame case-worthy items as unresolved problems or decisions, not predetermined solutions.
