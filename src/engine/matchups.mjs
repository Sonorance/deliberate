...
export async function matchupPrompt(store, project, competitor, { at = Date.now() } = {}) {
  ...
  const rivalBlock = `## The rival (STRICT)\nProduce a single-competitor head-to-head of **${rival}** against this project.\n- competitor: ${rival}\n- as_of: ${asOf}\n${tracked ? '- This rival is already tracked in competitors.md — use its details and monitoring sources as grounding.' : '- This rival is NOT yet tracked in competitors.md — research it first-party from scratch, then ask the user whether to add it to the project context (default yes): on yes, add its details and official monitoring sources only to competitors.md so `brief` starts tracking it; on no, proceed without touching context.'}\nSet the matchup's "As of" line to exactly "${asOf}" — just the date, no added parenthetical. Every factual claim links to a first-party source, dated on or before ${asOf}; steelman the rival before finding its openings.`;
