 ...
 * project files + attached external sources, then edits deliberate/context/{product.md,competitors.md,ecosystem.md}).
 *
...
export async function initPrompt(store, project) {
  ...
  const user = `Set up the project context for "${project.name}". Read files inside this project directly and use the attached external sources, then EDIT these three files in deliberate/context/ **directly** (they exist as scaffolds — replace the italic guidance with real, grounded content; never fabricate a section you can't ground). In-project files are automatic grounding, not source entries.\n\n### Attached external sources:${srcs}${repo}\n\n----- deliberate/context/product.md -----\n${product}\n\n----- deliberate/context/competitors.md -----\n${competitors}\n\n----- deliberate/context/ecosystem.md -----\n${ecosystem}`;
